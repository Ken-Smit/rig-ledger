...
interface Props {
  truck: Truck
  onSave: (id: string, data: Partial<TruckFormData>) => Promise<void>
  onClose: () => void
}
...
export default function EditTruckModal({ truck, onSave, onClose }: Props) {
  const [form, setForm] = useState({
    ...
    annual_inspection_date: toDateInput(truck.annual_inspection_date),
    brake_inspection_date: toDateInput(truck.brake_inspection_date),
    last_oil_change_date: toDateInput(truck.last_oil_change_date),
    last_oil_change_mileage: truck.last_oil_change_mileage != null ? String(truck.last_oil_change_mileage) : '',
    oil_change_interval: truck.oil_change_interval != null ? String(truck.oil_change_interval) : '',
    last_tire_rotation_date: toDateInput(truck.last_tire_rotation_date),
    coolant_flush_date: toDateInput(truck.coolant_flush_date),
    transmission_service_date: toDateInput(truck.transmission_service_date),
    tire_size: truck.tire_size ?? '',
    tire_brand: truck.tire_brand ?? '',
    tire_model: truck.tire_model ?? '',
  })
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError('')
    try {
      const payload: Partial<TruckFormData> = {
      ...
      await onSave(truck._id, payload)
      onClose()
    } catch {
    ...
    <div className="modal-backdrop" onClick={onClose}>
      <div className="modal-card" onClick={e => e.stopPropagation()}>
        ...
        <div className="modal-header">
          <span className="modal-title">EDIT UNIT</span>
        ...
        <form className="modal-form" onSubmit={handleSubmit}>
          ...
          <div className="modal-row">
            <div className="field-group">
              ...
              <input className="field-input" type="number" value={form.year} onChange={set('year')} required min={1900} max={2100} />
            </div>
            <div className="field-group">
          ...
          <div className="modal-row">
            <div className="field-group">
              ...
              <input className="field-input" type="text" value={form.make} onChange={set('make')} required />
            </div>
            <div className="field-group">
              <label className="field-label">MODEL</label>
              <input className="field-input" type="text" value={form.model} onChange={set('model')} required />
            </div>
          </div>
          <div className="field-group">
